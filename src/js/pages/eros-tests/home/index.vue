<template>
    <wxc-tab-page class="list-wrapper"
        ...
        <list v-for="(v,index) in tabContents"
            ...
            <cell class="cell-content" v-for="x in v.categoryContent">
...
<script>
    ...
    import streamContent from 'Components/categoryContent/streamContent.vue'
    ...
    export default {
        ...
        created(){
            this.$fetch({
                ...
                {
                    this.tabContents[i]={
                        dataSpm:this.tabTitles[i].dataSpm,
                        bannerItems:[],
                        categoryContent:[]
                    }
                }
                console.log(this.tabContents)
                this.selectTabPage(this.tabTitles[0].dataSpm)
            },error=>{
              console.log(error)
            })
        },
        methods:{
            wxcTabPageCurrentTabSelected(e){
                ...
                this.selectTabPage(this.tabContents[e.page].dataSpm)
            },
            onRefresh(index)
            {

            },
            selectTabPage(dataSpm){
                this.$fetch({
                    method:'GET',
                    name:'showTab'+dataSpm
                }).then(resData=>{
                    this.tabContents=this.tabContents.map((val,index)=>{
                        if(val.dataSpm===resData.data.dataSpm) {
                            return resData.data
                        }
                        return val
                    })
